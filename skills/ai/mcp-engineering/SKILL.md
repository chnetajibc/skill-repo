...
5. Evaluate: scripted happy-path + misuse + adversarial-input runs before shipping (see `references/upstream-mcp-builder/` evaluation pattern).
6. Verify: run the eval script, exercise tools from a real client, and confirm logs show who called what.

## References

- `references/upstream-mcp-builder/` (Anthropic, verbatim).
