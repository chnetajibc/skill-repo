...
Linting: enforce repo config; distinguish defects from style preference in review.
...
SOURCE: khasky-awesome-agent-skills/skills/awesome-code-standards
...
- Plain verbs in comment text — A closed list of borrowed diction never appears in a comment: the ceremonial verb where the plain one exists. The list and its replacements are the Borrowed diction section of `awesome-humanize-en/references/lexicon.md`, which also carries the domains where one of the words is the product's own vocabulary. Without that skill the rule still binds: where a plainer verb exists, the comment uses it. (Comment text only — inside a string literal or product data the word is content.)
